...
Lots of spatial analysis relies on data associated with specific administrative units.

There's lots of packages we can grab these sort of data from, e.g. `rnaturalearth` and `worlddatr`. 

Here's how you can retrieve national borders in Africa with `rnaturalearth`:
...
Here's how you can retrieve admin level-1s in Nigeria with the MAP package:
...
Say we have our own dataset, and we want to map that (highly likely I know!). We can join that dataset to our simple feature collection like so:
...
If we want to specify our own colours, that's easy too. I can also crop the map, e.g. to remove sub-Antarctic Marion Island:
