---
permalink: /teaching/
title: Teaching Resources
toc: true
---

Here's an archive of bits and bobs I've made up for workshops and tutoring. Added and updated sporadically.

## Mapping and spatial data in R

- <a href="/teaching/chloropleths_101">How to make a nice chloropleth map</a>, March 2026  
How to plot polygons and rasters in base graphics and `ggplot2`
- <a href="/teaching/map_demo">How much malaria is there?</a>, September 2025  
How to use the `malariaAtlas` package to extract MAP estimates of malaria transmission rate at locations in time and space
- <a href="/teaching/mapping_intro">Introduction to mapping in R</a>, January 2025  
An introduction to manipulating spatial data in R with `raster`, `sf`, and `terra`

## Visualisation and presentation skills

- ["Let's talk about visual clarity"](https://lu-harr.github.io/_pages/teaching/visualisation_workshop.pdf), July 2023  
Some introductory slides for a peer-to-peer workshop on visualising research at the Melbourne Math Bio away day
  - [v2: "Visualising data and other horror stories"](https://lu-harr.github.io/_pages/teaching/visualisation_workshop_v2.pdf), January 2026
  For the UCT-CCOAT MARC SE-Africa writing retreat
- <a href="/teaching/how_to_poster">How to give a great poster presentation</a>, August 2025  
Some resources for a workshop on making up a research poster
- [Lightning talks](https://lu-harr.github.io/_pages/teaching/3mt.pdf), April 2026  
Some *brief* and *basic* pointers on giving lightning talks in preparation for IDDO's inaugural lightning talk competition

## Miscellaneous

- <a href="/teaching/dojo_intro">IDDO Data Dojo 1</a> (outline only), June 2026
- <a href="/teaching/version_control">Introduction to version control with git and GitHub</a>, June 2026 
- <a href="/teaching/latex_and_overleaf">Introduction to academic writing with LaTeX and Overleaf</a>, April 2026 




<!-- [Foundations of Computing resources are not uploaded here but I'll put them in a private repo] -->
